...
import type { PhotoRecord, BlackboardData } from "@/lib/types";
...
export default function PhotoEditor({ photo, onClose, onSaved }: EditorProps) {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!blob) { toast("元画像が見つかりません", "error"); return; }
  ...
  // #8修正: マジック消しゴムをチャンク分割処理でUIフリーズ防止
  const applyMagicEraser = async () => {
    ...
    toast("消しゴムを適用しました", "success");
  };

  // #9修正: toBlob nullガード + 保存成功/失敗フィードバック
