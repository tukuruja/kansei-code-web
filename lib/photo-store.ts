...
export async function loadPhotosMeta(): Promise<PhotoRecord[]> {
  ...
  // IndexedDBになければlocalStorageから移行（#3: 旧データ互換）
  try {
...
function migratePhotos(photos: PhotoRecord[]): PhotoRecord[] {
  return photos.map((p) => {
    ...
    // 旧thumbnail DataURL → thumbnailKey移行（あれば）
    // 既存のthumbnailフィールドがDataURLの場合はそのまま残す（描画互換用）
    // 新規写真はthumbnailKeyでIndexedDBから読む
