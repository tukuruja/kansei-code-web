...
import type { PhotoRecord, PhotoCategory } from "@/lib/photo-types";
import { PHOTO_CATEGORIES, DATA_SCHEMA_VERSION } from "@/lib/photo-types";
import {
  loadPhotosMeta,
  savePhotosMeta,
  saveImage,
...
export default function PhotosPage() {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      await refreshPhotos();
      toast.success(`${selectedIds.size}枚を削除しました`);
    } catch {
  ...
  const handleUpload = async () => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        // 逐次メタデータ保存（途中でブラウザが閉じてもデータを失わない）
    ...
    if (errorList.length > 0) {
      toast.warning(`${errorList.length}件のエラーがありました`);
    }
    onUploaded();
  };
