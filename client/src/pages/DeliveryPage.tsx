...
import type { ProjectInfo, PhotoRecord } from "@/lib/photo-types";
import { DELIVERY_FOLDER_STRUCTURE, DELIVERY_REQUIRED_FIELDS, DELIVERY_FIELD_LABELS } from "@/lib/photo-types";
...
export default function DeliveryPage() {
  ...
  const handlePreviewXML = () => {
    ...
    if (missing.length > 0) { toast.warning(`未入力: ${missing.join("、")}`); return; }
    setXmlPreview(generatePhotoXML(photos, projectInfo));
  };

  // streamFiles + 枚数警告
  const handleExportZip = async () => {
    if (photos.length === 0) { toast.warning("写真が登録されていません"); return; }
    const missing = validateProject();
    if (missing.length > 0) { toast.warning(`未入力: ${missing.join("、")}`); return; }
    ...
    try {
      ...
      if (skipped > 0) toast.warning(`${skipped}枚の画像が見つからずスキップしました`);
      else toast.success("ZIP出力が完了しました");
    } catch (err) {
        ...
        <div className="fixed inset-0 bg-black/50 z-40 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-3xl max-h-[80vh] flex flex-col">
            ...
            <div className="p-4 border-t border-gray-200 dark:border-gray-700">
              <button onClick={() => { navigator.clipboard.writeText(xmlPreview); toast.success("コピーしました"); }}
